...
## Макеты в репозитории (ветка repo-mode)

Второй режим работы: макеты живут в `prototypes/` этого же репозитория, и каждый
пуш или PR, который их трогает, автоматически сверяется с дизайн-системой —
рабочий процесс `.github/workflows/prototypes-check.yml`.

Что это даёт по сравнению с «бот на ноутбуке»:

- история: видно, кто и когда менял макет, и как менялись расхождения
- проверка чужих правок: PR с макетом не вольётся, пока в нём новые расхождения
- бот работает при выключенном компьютере
- снимается вся возня с iCloud-защитой macOS

Чего этот режим не отменяет: слепок ДС по-прежнему снимает агент с Figma MCP
(Variables API — Enterprise) и коммитит `snapshots/ds-latest.json` в репо.
CI на нём только сверяет.

**Приватность.** Макеты — рабочие материалы заказчика. Этот режим включается
только в приватном репозитории; в публичном ветка несёт механику без данных.

Миграция: скопировать папки прототипов в `prototypes/`, выставить
`prototypesRoot: "prototypes"` в config.json, закоммитить config, слепок
и базовую линию, убрать `.bak`-файлы.
